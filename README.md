...
| `cllms.utility.modelIdByProvider` | `{}` | Route utility/helper requests to a cheaper API model on the same provider, keyed by provider id (e.g. `{ "qwen": "qwen-flash" }`). Empty by default (no downgrade). The pricing estimate still uses the selected model, but the provider bills the cheaper one. Prefer VS Code's native `chat.utilityModel` (`CLLMs: Configure Utility Model`) when available |
