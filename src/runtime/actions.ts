...
const ACTION_URLS: readonly ActionUrlDefinition[] = [
	{
		...
		handle: () => vscode.commands.executeCommand('cllms.setApiKey'),
		resolveFailureMessage: 'Failed to resolve CLLMs set API key URI',
	},
	{
		key: 'showLogs',
		path: SHOW_LOGS_URI_PATH,
		handle: () => logger.show(),
		resolveFailureMessage: 'Failed to resolve CLLMs show logs URI',
			...
			handleUri(uri) {
				...
				if (action) {
					void Promise.resolve(action.handle()).catch((error) => {
						logger.warn(`Failed to handle CLLMs URI action: ${uri.path}`, error);
					});
					return;
				}
				logger.warn(`Unhandled CLLMs URI: ${uri.toString(true)}`);
			},
		}),
	);
