...
} from '../../protocols/errors/diagnostics';
...
function formatVisionProxyTestDiagnostics(
	...
	return joinDiagnosticParts(
		`kind=vision`,
		`phase=describe`,
		`providerFamily=${safeDiagnosticString(config.providerFamily)}`,
		`apiType=${safeDiagnosticString(config.apiType)}`,
		`model=${safeDiagnosticString(config.modelId)}`,
		`endpoint=${safeDiagnosticString(config.url)}`,
		`hasApiKey=${Boolean(apiKey?.trim())}`,
		`responseChars=${description.length}`,
		config.headers
			? `headerNames=${formatDiagnosticValue(Object.keys(config.headers).sort())}`
			: undefined,
	);
}
