...
interface VscodeShim {
	...
	__state: { openedExternal: vscode.Uri[]; inputBoxes: Array<{ __buttonEmitter: { fire(v: unknown): void } }> };
}
...
describe('AuthManager.promptForApiKey', () => {
	...
	it('opens the provider API key page when the input-box button is triggered', async () => {
		...
		assert.equal(
			shim.__state.openedExternal[0]?.toString(),
			provider.externalUrls.apiKeys,
		);
	});
});
