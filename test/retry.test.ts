...
describe('getRetryDelayMs', () => {
	...
	it('honors Retry-After, capped at the max delay', () => {
		assert.equal(getRetryDelayMs(new LlmRequestError({ message: 'x', kind: 'http', retryAfterMs: 5_000 }), 1), 5_000);
		// 50s advised delay is clamped to the 20s ceiling.
		assert.equal(getRetryDelayMs(new LlmRequestError({ message: 'x', kind: 'http', retryAfterMs: 50_000 }), 1), 20_000);
	});
