...
describe('provider registry', () => {
	...
	it('gives each provider a unique secret key, host, and override setting', () => {
		...
		for (const provider of Object.values(PROVIDERS)) {
			assert.equal(secrets.has(provider.apiKeySecret), false, `duplicate secret ${provider.apiKeySecret}`);
			assert.equal(hosts.has(provider.officialHost), false, `duplicate host ${provider.officialHost}`);
			assert.equal(
...
describe('model registry', () => {
	...
	it('exposes the MiniMax models on the minimax provider; only M3 has native vision', () => {
		const minimaxIds = MODELS.filter((m) => m.provider === 'minimax').map((m) => m.id).sort();
		assert.deepEqual(minimaxIds, ['MiniMax-M2.7', 'MiniMax-M3']);
		...
		// M3 is natively multimodal; M2.7 is text-only and uses the vision proxy.
	...
	it('exposes the Xiaomi MiMo models on the xiaomi provider', () => {
		const xiaomiIds = MODELS.filter((m) => m.provider === 'xiaomi').map((m) => m.id).sort();
	...
	it('marks only the known native-vision models', () => {
		const visionIds = MODELS.filter((m) => m.capabilities.imageInput).map((m) => m.id).sort();
