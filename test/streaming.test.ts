...
function makeRequest(overrides: Partial<LlmRequest> = {}): LlmRequest {
	return { model: 'test-model', messages: [{ role: 'user', content: 'hello' }], stream: true, ...overrides };
}

function contentChunk(content: string): LlmStreamChunk {
	return { id: 'x', object: 'chat.completion.chunk', created: 0, model: 'test', choices: [{ index: 0, delta: { content }, finish_reason: null }] };
}

function thinkingChunk(text: string): LlmStreamChunk {
	return { id: 'x', object: 'chat.completion.chunk', created: 0, model: 'test', choices: [{ index: 0, delta: { reasoning_content: text }, finish_reason: null }] };
}

function usageChunk(u: { prompt_tokens: number; completion_tokens: number; total_tokens: number }): LlmStreamChunk {
	return { id: 'x', object: 'chat.completion.chunk', created: 0, model: 'test', choices: [], usage: u };
}

function toolCallChunk(index: number, id: string, name: string, args: string): LlmStreamChunk {
	return { id: 'x', object: 'chat.completion.chunk', created: 0, model: 'test', choices: [{ index: 0, delta: { tool_calls: [{ index, id, type: 'function', function: { name, arguments: args } }] }, finish_reason: null }] };
}

function finishChunk(reason: 'stop' | 'tool_calls' = 'stop'): LlmStreamChunk {
	return { id: 'x', object: 'chat.completion.chunk', created: 0, model: 'test', choices: [{ index: 0, delta: {}, finish_reason: reason }] };
}
...
function mockHttpErrorResponse(status: number, retryAfter?: string): Response {
	return {
		status, ok: false,
		statusText: status === 401 ? 'Unauthorized' : status === 429 ? 'Too Many Requests' : 'Server Error',
		headers: { get: (name: string) => name.toLowerCase() === 'retry-after' ? (retryAfter ?? null) : null },
		body: null, text: async () => JSON.stringify({ error: { message: 'mock error' } }),
...
function recordingCallbacks() {
	...
	const cbs: StreamCallbacks = {
		...
		onDone: () => { doneCount.value++; },
		onError: (e) => { errorCalls.push(e instanceof LlmRequestError ? `${e.kind}:${e.status ?? e.code}` : e.message); lastError.value = e; },
...
describe('LlmClient.streamChatCompletion', () => {
	...
	it('streams content chunks and calls onDone on [DONE]', async () => {
		globalThis.fetch = mock.fn(() => Promise.resolve({
			...
			headers: { get: () => null },
			body: buildStreamBody([sseFrame(contentChunk('Hello')), sseFrame(contentChunk(' world')), sseFrame('[DONE]')]),
			text: async () => '',
		} as unknown as Response));
	...
	it('streams thinking then content', async () => {
		globalThis.fetch = mock.fn(() => Promise.resolve({
			status: 200, ok: true, statusText: 'OK', headers: { get: () => null },
			body: buildStreamBody([sseFrame(thinkingChunk('reason…')), sseFrame(contentChunk('answer')), sseFrame('[DONE]')]),
			text: async () => '',
		} as unknown as Response));
	...
	it('reports usage via onUsage', async () => {
		globalThis.fetch = mock.fn(() => Promise.resolve({
			status: 200, ok: true, statusText: 'OK', headers: { get: () => null },
			body: buildStreamBody([sseFrame(usageChunk({ prompt_tokens: 10, completion_tokens: 5, total_tokens: 15 })), sseFrame('[DONE]')]),
			text: async () => '',
		} as unknown as Response));
	...
	it('aggregates tool call deltas', async () => {
		globalThis.fetch = mock.fn(() => Promise.resolve({
			status: 200, ok: true, statusText: 'OK', headers: { get: () => null },
			body: buildStreamBody([sseFrame(toolCallChunk(0, 'c1', 'search', '{"q":')), sseFrame(toolCallChunk(0, '', '', '"hi"}')), sseFrame('[DONE]')]),
			text: async () => '',
		} as unknown as Response));
	...
	it('does NOT retry DNS error (ENOTFOUND)', async () => {
		let attempts = 0;
		globalThis.fetch = mock.fn(() => { attempts++; return Promise.reject(createNetworkError('ENOTFOUND', 'dns')); });
		const client = new LlmClient(BASE_URL, API_KEY, { maxRetries: 2, idleTimeoutMs: 0 });
		const { cbs, lastError, errorCalls } = recordingCallbacks();
		await client.streamChatCompletion(makeRequest(), cbs);
	...
	it('does NOT retry TLS error (CERT_HAS_EXPIRED)', async () => {
		let attempts = 0;
		globalThis.fetch = mock.fn(() => { attempts++; return Promise.reject(createNetworkError('CERT_HAS_EXPIRED', 'tls')); });
		const client = new LlmClient(BASE_URL, API_KEY, { maxRetries: 2, idleTimeoutMs: 0 });
		const { cbs, lastError } = recordingCallbacks();
		await client.streamChatCompletion(makeRequest(), cbs);
	...
	it('cancellation during stream returns without onError', async () => {
		...
		globalThis.fetch = mock.fn(() => Promise.resolve({
			status: 200, ok: true, statusText: 'OK', headers: { get: () => null },
			body: buildStreamBody([sseFrame(contentChunk('first')), sseFrame(contentChunk('second')), sseFrame('[DONE]')]),
			text: async () => '',
		} as unknown as Response));
		...
		cbs.onContent = (t) => { origOnContent(t); cts.cancel(); };
	...
	it('does not retry after onThinking then mid-stream error', async () => {
		...
		globalThis.fetch = mock.fn(() => {
			...
			if (attempts === 1) {
				...
				return Promise.resolve({ status: 200, ok: true, statusText: 'OK', headers: { get: () => null }, body, text: async () => '' } as unknown as Response);
			}
			return Promise.resolve({
				status: 200, ok: true, statusText: 'OK', headers: { get: () => null },
	...
	it('DOES retry when only onUsage delivered (not visible output)', async () => {
		...
		globalThis.fetch = mock.fn(() => {
			...
			if (attempts === 1) {
				const encoder = new TextEncoder();
				const frame = encoder.encode(sseFrame(usageChunk({ prompt_tokens: 5, completion_tokens: 0, total_tokens: 5 })));
				let enqueued = false;
				const body = new ReadableStream<Uint8Array>({
					async pull(controller) {
						if (!enqueued) {
	...
	it('does not duplicate thinking/toolCall on retry', async () => {
		...
		globalThis.fetch = mock.fn(() => {
			...
			return Promise.resolve({
				status: 200, ok: true, statusText: 'OK', headers: { get: () => null },
				body: buildStreamBody([sseFrame(thinkingChunk('think')), sseFrame(toolCallChunk(0, 't1', 'fn', '{}')), sseFrame('[DONE]')]),
				text: async () => '',
			} as unknown as Response);
		});
	...
	it('ignores data after [DONE]', async () => {
		globalThis.fetch = mock.fn(() => Promise.resolve({
			status: 200, ok: true, statusText: 'OK', headers: { get: () => null },
			body: buildStreamBody([sseFrame(contentChunk('before')), sseFrame('[DONE]'), sseFrame(contentChunk('after'))]),
			text: async () => '',
		} as unknown as Response));
	...
	it('aggregates multi-index tool calls', async () => {
		globalThis.fetch = mock.fn(() => Promise.resolve({
			...
			body: buildStreamBody([
				sseFrame(toolCallChunk(0, 'ca', 'fa', '{"x":')), sseFrame(toolCallChunk(1, 'cb', 'fb', '{"y":')),
				sseFrame(toolCallChunk(0, '', '', '1}')), sseFrame(toolCallChunk(1, '', '', '2}')),
				sseFrame('[DONE]'),
			]),
			text: async () => '',
		} as unknown as Response));
